...
const handleNormalize = async () => {
    ...
    loading.value = true;
    try {
        const res = await API.normalizeWords(2000);
        if (res && res.success) {
            uni.showModal({
                title: '清洗完成',
                content: `共扫描 ${res.total || 0} 条，修复 ${res.updated || 0} 条`,
                showCancel: false
            });
        } else {
            uni.showModal({ title: '错误', content: res?.msg || '词库清洗失败', showCancel: false });
        }
    } catch (e) {
        uni.showModal({ title: '错误', content: e?.message || '词库清洗失败', showCancel: false });
    } finally {
        loading.value = false;
    }
};
