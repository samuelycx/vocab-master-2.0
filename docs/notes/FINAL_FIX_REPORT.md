...
```bash
# 检查emoji是否在text标签中
grep -rn "stat-icon\|action-icon" miniprogram/dist/dev/mp-weixin/components/Dashboard.wxml
...
```bash
# 检查是否使用新API
grep -n "getDeviceInfo\|getWindowInfo\|getAppBaseInfo" miniprogram/dist/dev/mp-weixin/state.js
